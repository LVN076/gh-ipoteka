 ...
 * - авторизация: id.vk.com/authorize?code_challenge=...&code_challenge_method=S256
 * - обмен токена: id.vk.com/oauth2/token с code_verifier
 * - user_id получаем из id.vk.com/oauth2/user_info
 *
 * code_verifier передаётся клиентом через state параметр: "vk_sub_check|<verifier>"
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Шаг 1: обменять code на access_token через VK ID с PKCE
    ...
    console.log('[vk-callback] Exchanging code, verifier length:', codeVerifier.length)
